...
stmt' :: Parser Stmt
stmt' = ifStmt <|> whileStmt <|> skipStmt <|> assignStmt <|> parens stmt
...
aTerm :: Parser AExpr
aTerm = parens aExpr <|> Var <$> identifier <|> IntConst <$> integer

bTerm :: Parser BExpr
bTerm = parens bExpr <|> (BoolConst True <$ rword "true") <|> (BoolConst False <$ rword "false") <|> rExpr
...
relation = (Greater <$ symbol ">") <|> (Less <$ symbol "<")

divider :: String
divider = concat $ replicate 50 "-"
